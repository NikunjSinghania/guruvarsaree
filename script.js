...
const promise = new Promise((res, rej)=>{
        ...
        function loadImages(i, brand){
            ...
            for(let count=1;count<=8;count++){
                ...
                imgs.setAttribute('data-src', brand+'/100'+count+'-min.jpg')
...
window.addEventListener('scroll', (e)=>{
    ...
    if( tempActive < length && document.querySelector('.section'+(tempActive+1)).getBoundingClientRect().y <= 0){
        if(tempActive != 0)
        {
            ...
            for(let i=0;i<8;i++){
                let s = document.querySelectorAll('.section'+tempActive+' img')[i].attributes['data-src'].value
                document.querySelectorAll('.section'+tempActive+' img')[i].src = s
            }
